...
WITH active_patients AS (
	WITH admission_date AS (
		...
		FROM bed_management_locations_view
		ORDER BY patient_id, visit_id, start_date),
	discharge_date AS (	
		SELECT
			DISTINCT ON (patient_id, visit_id) patient_id,
			visit_id,
			discharge_date AS discharge_date
		FROM bed_management_locations_view bmlv
...
range_values AS (
	SELECT 
		date_trunc('day', min(ap.admission_date)) AS minval,
...
daily_admissions AS (
	SELECT 
		date_trunc('day', ap.admission_date) AS day,
		count(*) AS patients
	FROM active_patients AS ap
	GROUP BY 1),
daily_exits AS (
